...
/**
 * This file is also part of MartianNet.
 *
...
print_data(char* buffer, u_int length)
{
	...
	//sscanf(s_buffer, "   |  %.*s\n", link);
	strncpy(link, buffer, length);
	link[length] = '\0';
	//sscanf(s_buffer, "get:%s:%d", link, layers);
	printf("%s\n", link);
	printf("length: %d\n", length);
	bytes_written = write(pipe_out, buffer, (int)length);
	if(bytes_written == -1)
	{
			fprintf(stderr, "Error while writing to ");
			fprintf(stderr, PIPE_OUT);
			fprintf(stderr, "\n");
			exit(1);
	}
